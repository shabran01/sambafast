  ...
  if ($userType == 'OneTimeUser') {
    ...
    if ($response_code == "0") {
      ...
      if ($rechargeResult === false) {
        ...
        error_log("Mpesa: Payment successful but package activation FAILED for user: " . $PaymentGatewayRecord->username . ", Amount: " . $amount_paid . ", Mpesa Code: " . $mpesa_code);
        ...
        error_log("Mpesa: Duplicate recharge prevented for user: " . $PaymentGatewayRecord->username . ", Amount: " . $amount_paid . ", Mpesa Code: " . $mpesa_code);
        ...
        if (class_exists('TransactionDuplicateHelper')) {
            TransactionDuplicateHelper::logDuplicateAttempt(
                ...
                $amount_paid,
                'mpesa',
                'Duplicate prevented in mpesa callback (rechargeUser returned duplicate)'
            );
        }
        ...
        error_log("Mpesa: Payment successful for user: " . $PaymentGatewayRecord->username . ", Amount: " . $amount_paid . ", Mpesa Code: " . $mpesa_code);
      }
    }
  } else {
    ...
    if ($OFFLineMode['value'] == 1) {
      ...
      if ($resultDesc == "Confirmation Service not accepted" || $resultCode == 1) {
        ...
        $rechargeResult = Package::rechargeUser($user['id'], $trx['routers'], $trx['plan_id'], $trx['gateway'], 'mpesa');
        if ($rechargeResult === false) {
          _log('[' . $resultDesc . ']: M-Pesa ' . "Payment Successful, But Failed to activate your Package" . json_encode($callbackData));
        } elseif ($rechargeResult === 'duplicate') {
          _log('[' . $resultDesc . ']: M-Pesa ' . "Payment Successful (duplicate callback, package not re-activated)" . json_encode($callbackData));
        }
        _log('[' . $resultDesc . ']: M-Pesa ' . "Payment Successful" . json_encode($callbackData));
