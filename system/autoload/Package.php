...
class Package
{
    ...
    public static function rechargeUser($id_customer, $router_name, $plan_id, $gateway, $channel, $note = '')
    {
        ...
        // Load TransactionDuplicateHelper if it exists
        ...
        try {
            $recent = ORM::for_table('tbl_transactions')
                ...
                ->where('plan_name', $p['name_plan'])
                ->where('recharged_on', $date_only)
                ->where_gte('recharged_time', date('H:i:s', strtotime('-60 seconds')))
                ->find_one();
            if ($recent) {
                return 'duplicate';
            }
        ...
        if (isset($zero) && $zero == 1) {
            ...
            // Add PPPoE specific information to comment
            if ($p['type'] == 'PPPOE' && !$isVoucher) {
                // Add phone number
                if (!empty($c['phonenumber'])) {
                    $note .= "Phone: " . $c['phonenumber'] . "\n";
                }
                ...
                if ($p['validity_unit'] == 'Period') {
                    ...
                    $note .= "Expires on: $expiry_date\n";
                } else {
                    // For validity units like Months, Hours, etc.
                    $expiry_date = date('d M Y H:i', strtotime("+$p[validity] $p[validity_unit]"));
                    $note .= "Expires on: $expiry_date\n";
                }
                
                // Add bandwidth information
                if (!empty($p['id_bw'])) {
                    $bandwidth_name = self::getBandwidthName($p['id_bw']);
                    $note .= "Bandwidth: $bandwidth_name\n";
                }
            }
        }
        ...
        if ($b) {
            ...
            if (class_exists('TransactionDuplicateHelper') && !$isVoucher) {
                $transaction_price = ($p['validity_unit'] == 'Period') ? 
                    (empty($add_inv) ? $p['price'] + $add_cost : $add_inv + $add_cost) : 
                    $p['price'] + $add_cost;
                
                if (TransactionDuplicateHelper::isDuplicateTransaction($c['username'], $p['name_plan'], $transaction_price, $gateway, $channel)) {
                    TransactionDuplicateHelper::logDuplicateAttempt($c['username'], $p['name_plan'], $transaction_price, $gateway, 'Duplicate prevented in Package::rechargeUser');
                    return false; // Prevent duplicate transaction
                }
            }
    ...
    public static function getBandwidthName($bw_id)
    {
        $bw = ORM::for_table('tbl_bandwidth')->select("name_bw")->find_one($bw_id);
        return $bw ? $bw['name_bw'] : 'Unknown';
    }
