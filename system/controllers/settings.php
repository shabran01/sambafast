...
switch ($action) {
    ...
    case 'app':
        ...
        if (!empty(_get('testWa'))) {
            $result = Message::sendWhatsapp(_get('testWa'), 'SpeedRadius Test Whatsapp');
            r2(U . "settings/app", 's', 'Test Whatsapp has been send<br>Result: ' . $result);
        }
        if (!empty(_get('testSms'))) {
            $result = Message::sendSMS(_get('testSms'), 'SpeedRadius Test SMS');
            r2(U . "settings/app", 's', 'Test SMS has been send<br>Result: ' . $result);
        }
        if (!empty(_get('testEmail'))) {
            Message::sendEmail(_get('testEmail'), 'SpeedRadius Test Email', 'SpeedRadius Test Email Body');
            r2(U . "settings/app", 's', 'Test Email has been send');
        }
        if (!empty(_get('testTg'))) {
            $result = Message::sendTelegram('SpeedRadius Test Telegram');
    ...
    case 'app-post':
        ...
        if ($_POST['general'] && $company == '') {
            ...
            //checkbox
            $checks = ['hide_mrc', 'hide_tms', 'hide_aui', 'hide_al', 'hide_uet', 'hide_vs', 'hide_pg', 'hide_income_today', 'hide_income_month'];
    ...
    case 'users':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin', 'Agent', 'Viewer'])) {
            _alert(Lang::T('You do not have permission to access this page'), 'danger', "dashboard");
        }
        
        // If user is Viewer, ensure they can only view
        if($admin['user_type'] == 'Viewer') {
            $ui->assign('_admin_viewer', true);
        }
    ...
    case 'users-add':
        ...
        break;
    case 'users-view':
        $ui->assign('_title', Lang::T('View User'));
        $id  = $routes['2'];
        if (empty($id)) {
            $id = $admin['id'];
        }
    ...
    case 'users-edit':
        ...
        if ($admin['user_type'] == 'Viewer') {
            ...
            if ($d) {
                $ui->assign('id', $id);
                $ui->assign('d', $d);
                $ui->assign('isViewer', true); // Flag to disable form fields
                $csrf_token = Csrf::generateAndStoreToken();
                $ui->assign('csrf_token', $csrf_token);
    ...
    case 'users-edit-post':
        // Prevent Viewer role from making changes
        if ($admin['user_type'] == 'Viewer') {
            r2(U . 'settings/users', 'e', 'Viewer role has read-only access and cannot make changes');
        }
        ...
        $valid_types = ['SuperAdmin', 'Admin', 'Report', 'Agent', 'Sales', 'Viewer'];
        if (!in_array($user_type, $valid_types)) {
            r2(U . 'settings/users-edit/', 'e', 'Invalid user type');
            exit;
        }
    ...
    case 'notifications-post':
        ...
        $checkbox_fields = [
            'send_expired_to_pppoe',
            'send_expired_to_hotspot',
            'send_reminder_to_pppoe',
            'send_reminder_to_hotspot',
            'send_payment_to_pppoe',
            'send_payment_to_hotspot'
        ];
        
        foreach ($checkbox_fields as $field) {
            if (!isset($_POST[$field])) {
                $_POST[$field] = '0'; // Explicitly set to 0 when unchecked
            }
        }
