    ...
    if ($tur) {
        ...
        if ($tur['status'] != 'on') {
            ...
            if (!empty($user['phonenumber'])) {
                $ext_msg = $config['extend_confirmation'];
                if (empty($ext_msg)) {
                    $ext_msg = Lang::T("Your service has been extended until") . " " . Lang::dateAndTimeFormat($expiration, $tur['time']);
                } else {
                    // Start of simple template replacement if needed
                    $ext_msg = str_replace(
                        ['[[name]]', '[[expiration]]', '[[plan]]'],
                        [$user['fullname'], Lang::dateAndTimeFormat($expiration, $tur['time']), $p['name_plan']],
                        $ext_msg
                    );
                }
                Message::sendSMS($user['phonenumber'], $ext_msg);
                Message::sendWhatsapp($user['phonenumber'], $ext_msg);
            }
...
$monthly_usage = [];
try {
    $monthly_helper = dirname(__FILE__) . '/../helpers/monthly_usage.php';
    if (file_exists($monthly_helper)) {
        require_once $monthly_helper;
        $raw_usage = monthly_usage_get($user['id'], 12);
        foreach ($raw_usage as $row) {
            $is_current = ($row['month'] == date('Y-m'));
            $total_bytes = (int)$row['download_bytes'] + (int)$row['upload_bytes'];
            $monthly_usage[] = [
                'month'        => $row['month'],
                'month_label'  => date('F Y', strtotime($row['month'] . '-01')),
                'download'     => monthly_usage_format_bytes($row['download_bytes']),
                'upload'       => monthly_usage_format_bytes($row['upload_bytes']),
                'total'        => monthly_usage_format_bytes($total_bytes),
                'download_raw' => (int)$row['download_bytes'],
                'upload_raw'   => (int)$row['upload_bytes'],
...
$ui->assign('monthly_usage', $monthly_usage);
