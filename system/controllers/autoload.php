...
switch ($action) {
    case 'pool':
        ...
        // Always ensure EXPIRED-POOL is available for the selected router
        if (!empty($routers)) {
            $expiredPoolExists = false;
            foreach ($d as $pool) {
                if ($pool->pool_name == 'EXPIRED-POOL') {
                    $expiredPoolExists = true;
                    break;
                }
            }
            
            // If EXPIRED-POOL doesn't exist for this router, create it
            if (!$expiredPoolExists) {
                $expiredPool = ORM::for_table('tbl_pool')
                    ->where('pool_name', 'EXPIRED-POOL')
                    ->where('routers', $routers)
                    ->find_one();
                    
                if (!$expiredPool) {
                    // Create EXPIRED-POOL for this router
                    $expiredPool = ORM::for_table('tbl_pool')->create();
                    $expiredPool->pool_name = 'EXPIRED-POOL';
                    $expiredPool->range_ip = '90.0.0.2-90.0.0.254';
                    $expiredPool->local_ip = '90.0.0.1';
                    $expiredPool->routers = $routers;
                    $expiredPool->save();
                    
                    // Add to MikroTik router
                    try {
                        $mikrotikPppoe = new MikrotikPppoe();
                        $mikrotikPppoe->add_pool($expiredPool);
                        _log("Auto-created EXPIRED-POOL for router: $routers via admin interface");
                    } catch (Exception $e) {
                        _log("Error auto-creating EXPIRED-POOL for router $routers: " . $e->getMessage());
                    }
                }
                
                // Reload pools to include the newly created one
                $d = ORM::for_table('tbl_pool')->where('routers', $routers)->find_many();
            }
        }
