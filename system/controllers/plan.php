...
switch ($action) {
    case 'sync':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin', 'Viewer'])) {
        ...
        // Display sync interface with progress (count is scoped to both filters)
    ...
    case 'sync-process':
        ...
        // count_only: just return the total for that router/type (used by dropdown change event)
            ...
            try {
                ...
                if ($_app_stage != 'demo') {
                    if (file_exists($dvc)) {
                        ...
                        $results[] = ['username' => $tur['username'], 'status' => 'error', 'message' => 'File missing'];
                        $errors++;
                    }
                } else {
                    $results[] = ['username' => $tur['username'], 'status' => 'success', 'message' => 'Demo'];
                    $success++;
                }
            } catch (Exception $e) {
                $results[] = ['username' => $tur['username'] ?? '?', 'status' => 'error', 'message' => $e->getMessage()];
                $errors++;
            }
        }
    ...
    case 'view':
        ...
        // Get customer data for the template
        $customer = ORM::for_table('tbl_customers')->where('username', $in['username'])->find_one();
    ...
    case 'bulk-delete':
        ...
        $ids = _post('ids');
        if (empty($ids)) {
            r2(U . 'plan/list', 'e', Lang::T('No items selected'));
        }
        ...
        _log('[' . $admin['username'] . ']: ' . 'Bulk deleted ' . $count . ' plans', $admin['user_type'], $admin['id']);
        
        if ($failed > 0) {
            r2(U . 'plan/list', 'w', Lang::T('Deleted') . ' ' . $count . ' ' . Lang::T('items') . '. ' . $failed . ' ' . Lang::T('items could not be deleted'));
        } else {
            r2(U . 'plan/list', 's', Lang::T('Deleted') . ' ' . $count . ' ' . Lang::T('items successfully'));
        }
        break;
    ...
    case 'voucher':
        ...
        $query = ORM::for_table('tbl_plans')
            ->inner_join('tbl_voucher', array('tbl_plans.id', '=', 'tbl_voucher.id_plan'))
            ->select_many(
                'tbl_plans.*',
                'tbl_voucher.*',
                array(
                    'create_date' => 'tbl_voucher.create_date'
                )
            )
            ->where_not_null('tbl_voucher.id')
            ->where_not_equal('tbl_voucher.id', '');
            
        // Add create_date to the query if not exists
        if (!ORM::for_table('tbl_voucher')->raw_query('SHOW COLUMNS FROM tbl_voucher LIKE "create_date"')->find_one()) {
            ORM::raw_execute("ALTER TABLE tbl_voucher ADD COLUMN create_date DATETIME DEFAULT CURRENT_TIMESTAMP");
        }
        ...
        // Ensure we only get valid vouchers
        $query->where_not_null('tbl_voucher.code')
              ->where_not_equal('tbl_voucher.code', '');
        ...
        switch ($sort) {
            case 'id_asc':
                $query->order_by_asc('tbl_voucher.id');
                break;
            case 'id_desc':
                $query->order_by_desc('tbl_voucher.id');
                break;
            case 'name_asc':
                $query->order_by_asc('tbl_plans.name_plan');
                break;
            case 'name_desc':
                $query->order_by_desc('tbl_plans.name_plan');
                break;
            case 'create_date_asc':
                $query->order_by_asc('tbl_voucher.create_date');
                break;
            case 'create_date_desc':
                $query->order_by_desc('tbl_voucher.create_date');
                break;
            default:
                $query->order_by_desc('tbl_voucher.id');
                break;
    ...
    case 'print-voucher-pos':
        ...
        $ui->assign('_title', Lang::T('POS Print Vouchers'));
        $ui->assign('_c', $config);
        $ui->assign('from_id', $from_id);
        $ui->assign('limit', $limit);
        $ui->assign('planid', $planid);
        ...
        foreach ($v as $vs) {
            $voucher[] = [
                'code' => $vs['code'],
                'plan' => $vs['name_plan'],
                'price' => Lang::moneyFormat($vs['price']),
                'counter' => $n
            ];
            $n++;
        }
        ...
        $ui->assign('voucher', $voucher);
        $ui->assign('vc', $vc);
        run_hook('view_print_voucher_pos'); #HOOK
        $ui->display('print-voucher-pos.tpl');
        break;
    case 'voucher-post':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin', 'Agent', 'Sales'])) {
            _alert(Lang::T('You do not have permission to access this page'), 'danger', "dashboard");
    ...
    case 'voucher-bulk-delete':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin'])) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => Lang::T('You do not have permission to access this page')]);
            exit;
        }
        ...
        if (empty($ids)) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => Lang::T('No vouchers selected')]);
            exit;
        }
        ...
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            header('Content-Type: application/json');
            echo json_encode(['success' => true, 'message' => Lang::T('Successfully deleted') . " $count " . Lang::T('vouchers')]);
            exit;
        } else {
            r2(U . 'plan/voucher', 's', Lang::T('Successfully deleted') . " $count " . Lang::T('vouchers'));
        }
        break;

    case 'cleanup-vouchers':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin'])) {
            _alert(Lang::T('You do not have permission to access this page'), 'danger', "dashboard");
        }

        // First clean up vouchers with no codes
        $count1 = ORM::for_table('tbl_voucher')
            ->where_null('code')
            ->delete_many();

        // Clean up vouchers with empty codes
        $count2 = ORM::for_table('tbl_voucher')
            ->where('code', '')
            ->delete_many();

        // Clean up vouchers with no plan ID
        $count3 = ORM::for_table('tbl_voucher')
            ->where_null('id_plan')
            ->delete_many();

        // Clean up vouchers with invalid plan IDs
        $valid_plans = ORM::for_table('tbl_plans')->select('id')->find_array();
        $plan_ids = array_column($valid_plans, 'id');
        
        if (!empty($plan_ids)) {
            $count4 = ORM::for_table('tbl_voucher')
                ->where_not_in('id_plan', $plan_ids)
                ->delete_many();
        }

        $total = $count1 + $count2 + $count3 + (isset($count4) ? $count4 : 0);
        r2(U . 'plan/voucher', 's', Lang::T('Cleaned up') . " $total " . Lang::T('invalid vouchers'));
        break;
    ...
    case 'deduct':
        ...
        $ui->assign('_title', Lang::T('Subtract Balance'));
        $ui->assign('c', $c);
        $ui->display('deduct.tpl');
        break;
    case 'deduct-post':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin'])) {
            _alert(Lang::T('You do not have permission to access this page'), 'danger', "dashboard");
        ...
        if ($amount <= 0) {
            r2(U . 'plan/deduct/' . $id_customer, 'e', Lang::T('Amount must be greater than zero'));
        }

        if ($c['balance'] < $amount) {
            r2(U . 'plan/deduct/' . $id_customer, 'e',
                Lang::T('Insufficient balance. Current balance: ') . Lang::moneyFormat($c['balance']));
        }

        // Subtract balance WITHOUT inserting into tbl_transactions
        // so income reports and dashboard income figures are NOT affected
        Balance::min($id_customer, $amount);

        r2(U . 'customers/view/' . $id_customer, 's',
            Lang::moneyFormat($amount) . ' ' . Lang::T('has been subtracted from balance successfully'));
        break;
    case 'extend':
        $id = $routes[2];
        $days = $routes[3];
        ...
        break;
    case 'remove-used-vouchers':
        if (!in_array($admin['user_type'], ['SuperAdmin', 'Admin'])) {
            _alert(Lang::T('You do not have permission to access this page'), 'danger', "dashboard");
        }

        try {
            $count = ORM::for_table('tbl_voucher')
                ->where('status', '1')
                ->delete_many();

            if($count > 0){
                _alert(Lang::T('Successfully deleted ' . $count . ' used vouchers'), 'success', $_url . 'plan/voucher/');
            } else {
                _alert(Lang::T('No used vouchers found to delete'), 'info', $_url . 'plan/voucher/');
            }
        } catch (Exception $e) {
            _alert(Lang::T('An error occurred while deleting used vouchers'), 'danger', $_url . 'plan/voucher/');
        }
        break;
    case 'list':
    case 'csv':
        $is_csv = ($action == 'csv');
        $ui->assign('_title', Lang::T('Active Customers'));
        $search = _post('search');
        $status = _req('status');
        $router = _req('router');
        $plan = _req('plan');
        ...
        $query = ORM::for_table('tbl_user_recharges')
            ...
            ->left_outer_join('tbl_plans', array('tbl_user_recharges.plan_id', '=', 'tbl_plans.id'))
        ...
        if ($is_csv) {
            // CSV export — no pagination, get all matching rows
            $data = $query->find_array();
            header('Content-Type: text/csv; charset=utf-8');
            header('Content-Disposition: attachment; filename=plan_list_' . date('Y-m-d') . '.csv');
            $out = fopen('php://output', 'w');
            fputcsv($out, ['Username', 'Full Name', 'Plan', 'Price', 'Type', 'Created', 'Expires', 'Method', 'Router', 'Status']);
            foreach ($data as $row) {
                fputcsv($out, [
                    $row['username'],
                    ...
                    $row['plan_price'] ?? '0.00',
                    $row['type'],
                    $row['recharged_on'] . ' ' . $row['recharged_time'],
                    $row['expiration'] . ' ' . $row['time'],
                    $row['method'],
                    $row['routers'],
                    $row['status'] == 'on' ? 'Active' : 'Expired'
                ]);
            }
            fclose($out);
            die();
