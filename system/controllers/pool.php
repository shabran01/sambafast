    ...
    case 'sync':
        ...
        foreach ($pools as $pool) {
            if ($pool['routers'] != 'radius') {
                try {
                    $result = (new MikrotikPppoe())->update_pool($pool, $pool);
                    if ($result !== false) {
                        $log .= '✓ SUCCESS: ' . $pool['pool_name'] . ': ' . $pool['range_ip'] . '<br>';
                        $successCount++;
                    } else {
                        $log .= '✗ FAILED: ' . $pool['pool_name'] . ': Router connection failed<br>';
                        $failureCount++;
                    }
                } catch (\Exception $e) {
