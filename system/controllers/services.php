...
switch ($action) {
    case 'sync':
        ...
        if ($routes['2'] == 'hotspot') {
            ...
            foreach ($plans as $plan) {
                ...
                if ($_app_stage != 'demo') {
                    if (file_exists($dvc)) {
                        require_once $dvc;
                        try {
                            $result = (new $plan['device'])->add_plan($plan);
                            if ($result !== false) {
                                $log .= "✓ SUCCESS: $plan[name_plan], $plan[device]<br>";
                                $successCount++;
                            } else {
                                $log .= "✗ FAILED: $plan[name_plan], $plan[device] | Router Connection Failed<br>";
                                $failureCount++;
                            }
                        } catch (\Exception $e) {
                            $log .= "✗ ERROR: $plan[name_plan], $plan[device] | " . $e->getMessage() . "<br>";
                            $failureCount++;
                        }
                    } else {
            ...
            $summary = "<strong>Summary:</strong> {$successCount} successful, {$failureCount} failed<br><br>";
            r2(U . 'services/hotspot', 's', $summary . $log);
            ...
            foreach ($plans as $plan) {
                // Auto-fix: PPPoE plans must use MikrotikPppoe device, not MikrotikHotspot
                if (empty($plan['device']) || $plan['device'] === 'MikrotikHotspot') {
                    $plan->device = 'MikrotikPppoe';
                    $plan->save();
                    $log .= "⚠ AUTO-FIXED device to MikrotikPppoe for plan: $plan[name_plan]<br>";
                }
                $dvc = Package::getDevice($plan);
                if ($_app_stage != 'demo') {
                    if (file_exists($dvc)) {
            ...
            $summary = "<strong>Summary:</strong> {$successCount} successful, {$failureCount} failed<br><br>";
            r2(U . 'services/pppoe', 's', $summary . $log);
        }
    ...
    case 'hotspot':
        ...
        if ($sort == 'price_asc') {
            $query->order_by_expr('tbl_plans.price + 0 ASC');
        } elseif ($sort == 'price_desc') {
            $query->order_by_expr('tbl_plans.price + 0 DESC');
        }
        ...
        if ($msg == '') {
            ...
            if ($_app_stage != 'demo') {
                if (file_exists($dvc)) {
                    require_once $dvc;
                    $device = new $d['device']();
                    $device->update_plan($old, $d);

                    // Re-apply the updated plan to currently connected customers
                    // so speed/time/data changes reach users who are already online.
                    $changed = (
                        $old['id_bw'] != $d['id_bw'] ||
                        $old['name_plan'] != $d['name_plan'] ||
                        $old['typebp'] != $d['typebp'] ||
                        $old['limit_type'] != $d['limit_type'] ||
                        $old['time_limit'] != $d['time_limit'] ||
                        $old['time_unit'] != $d['time_unit'] ||
                        $old['data_limit'] != $d['data_limit'] ||
                        $old['data_unit'] != $d['data_unit'] ||
                        $old['shared_users'] != $d['shared_users']
                    );
                    if ($changed) {
                        ...
                        foreach ($active_recharges as $recharge) {
                            ...
                            try {
                                $device->add_customer($customer, $d);
                            } catch (\Exception $e) {
                                _log("Failed to re-apply plan {$d['name_plan']} to customer {$customer['username']}: " . $e->getMessage());
                            }
                        }
                    }
                } else {
        ...
        if ($msg == '') {
            ...
            if ($_app_stage != 'demo') {
                if (file_exists($dvc)) {
                    ...
                    // Re-apply the updated plan to currently connected PPPoE customers
                    // so speed/pool changes reach users who are already online.
                    $changed = (
                        $old['id_bw'] != $d['id_bw'] ||
                        $old['name_plan'] != $d['name_plan'] ||
                        $old['pool'] != $d['pool']
